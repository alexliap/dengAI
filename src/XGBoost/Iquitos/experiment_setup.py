...
def run_experiment(window: int, shift: int, drop_threshold: float, train_amount: float,
    ...
    anomaly_reg = anomaly_regressor_training(new_x_train, new_y_train, new_x_test, new_y_test, n_estimators = 20,
                                             max_depth = 4, learning_rate = 0.1, verbose = False)
    ...
    with mlflow.start_run():
        ...
        train_preds = predictions(model_1, anomaly_reg, df_x_train, anomaly_weight = 1.8, index = 1)
        test_preds = predictions(model_1, anomaly_reg, df_x_test, anomaly_weight = 1.8, index = 1)

        train_mae = np.round(np.mean(np.abs(train_preds - df_y_train.values)), 3).item()
        test_mae = np.round(np.mean(np.abs(test_preds - df_y_test.values)), 3).item()
        # --------------- Logging --------------- #
        ...
        train_title = (f'Train MAE: {train_mae} \n',
                       ...
                       f'Predictions Mean: {np.round(np.mean(train_preds), 3).item()}')
        axs[0].set_title('\n'.join(train_title))
        ...
        test_title = (f'Test MAE: {test_mae} \n',
                      ...
                      f'Predictions Mean: {np.round(np.mean(test_preds), 3).item()}')
        axs[1].set_title('\n'.join(test_title))
